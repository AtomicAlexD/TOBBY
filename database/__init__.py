...
class DatabaseManager:
    ...
    async def add_category(self, guild_id: int, category_name: str, category_description: str) -> str:
        """
        ...
        """
#        print(f'''INSERT INTO categories(guild_id, category_name, category_description)
#VALUES ({guild_id}, {category_name}, {category_description})''')
        await self.connection.execute(
            "INSERT INTO categories(guild_id, category_name, category_description) VALUES (?, ?, ?)",
                (
                    guild_id,
                    category_name,
        ...
        rows = await self.connection.execute(
            "SELECT category_name FROM categories WHERE guild_id=? ORDER BY id DESC LIMIT 1",(guild_id)
        )
        async with rows as cursor:
            new_category = await cursor.fetchone()
            return new_category
